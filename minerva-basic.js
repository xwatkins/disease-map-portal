...
async function fetchOverProxy(url) {
  return await fetch(
    "https://minerva-dev.lcsb.uni.lu/minerva-proxy/?url=" +
      url.replace("&", "%26")
  );
}
...
function computeCoordinatesTransformationData(options) {
  return {
    ...
    zoomFactor:
      ...
      (options.tileSize / (1 << options.minZoom))
  };
}
...
function pointToLonLat(point) {
  ...
  let lng =
    (x - transformationData.tileSize / 2) /
    transformationData.pixelsPerLonDegree;
  let latRadians =
    (y - transformationData.tileSize / 2) /
    -transformationData.pixelsPerLonRadian;
...
function fromProjectionToPoint(projection) {
  ...
  let x =
    transformationData.tileSize / 2 +
    lng * transformationData.pixelsPerLonDegree;
...
async function createMap({ elementId, projectId, submapId, serverUrl }) {
  ...
  response = await fetchOverProxy(
    apiUrl + "projects/" + projectId + "/models/" + submapId
  );
  ...
  response = await fetchOverProxy(
    apiUrl + "projects/" + projectId + "/overlays/"
  );
  ...
  let map = new ol.Map({
    ...
    view: new ol.View({
      center: ol.proj.fromLonLat(
        pointToLonLat({ x: width / 2, y: height / 2 })
      ),
      zoom: 4,
  ...
  map.on("click", async function(evt) {
    ...
    let response = await fetchOverProxy(
      apiUrl +
        "projects/" +
        projectId +
        "/models/" +
        submapId +
        "/bioEntities:search?coordinates=" +
        point.x +
        "," +
        point.y +
        "&count=1"
    ...
    if (data.length > 0) {
      if (data[0].type === "ALIAS") {
        ...
        let element = data[0];
        if (
          element.bounds.x <= point.x &&
          element.bounds.y <= point.y &&
          element.bounds.x + element.bounds.width >= point.x &&
          element.bounds.y + element.bounds.height >= point.y
        ) {
          ...
          feature.setStyle(
            new ol.style.Style({
              image: new ol.style.Icon({
                src:
                  "https://cdn.mapmarker.io/api/v1/pin?text=P&size=50&hoffset=1",
                anchor: [0.5, 1],
          ...
          map
            .getTargetElement()
            .dispatchEvent(
              new CustomEvent("map-click", { bubbles: true, detail: data })
            );

          console.log(
            "You clicked on element " + element.type + ": " + element.name
          );
        }
        ...
        response = await fetchOverProxy(
          apiUrl +
            ...
            "/models/" +
            submapId +
            "/bioEntities/reactions/?id=" +
            data[0].id
        );
        data = await response.json();
        console.log(
          "You clicked on reaction " + data[0].type + ": " + data[0].reactionId
        );
      }
    }
  });
